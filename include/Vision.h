...
namespace ORB_VISLAM
{
	class Vision
	{
		public:	
			...
			cv::Mat T_cam = cv::Mat::eye(4, 4, CV_32F);
			cv::Mat T_prev	= cv::Mat::eye(4, 4, CV_32F);
			
			cv::Mat Homography_Matrix = cv::Mat::eye(3, 3, CV_64F);
			cv::Mat Essential_Matrix = cv::Mat::eye(3, 3, CV_64F);
		...
		private:
			...
			void calcPoseHomography(cv::Mat &H);
			void calcPoseHomog(cv::Mat &Homog);
			void calcPoseEssMatrix(std::vector<cv::Point2f> &src, std::vector<cv::Point2f> &dst);
			void calcPose(std::vector<cv::Point2f> &src, std::vector<cv::Point2f> &dst);
